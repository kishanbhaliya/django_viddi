...
from ..models import (TakenInterview, Review, Settings,
                      PracticeInterview, Interviewee, Answer)
...
class CandidateInterviewListView(ListView):
    ...
    def get_queryset(self):
        if self.request.user.user_type == 10:
            queryset = self.request.user.user_for_interview.candidate.filter(status__in=[
                                                                             1, 2])
        else:
    ...
    def get_context_data(self, *args, **kwargs):
        context = super(CandidateInterviewListView,
        ...
        if len(interview_instance) > 0:
            ...
            if totalquestion > 0:
                ...
                for index, qu in enumerate(interview_instance.answers.filter(video_id='').order_by('index')):
                    qu_json.append([str(qu.question.text), int(
                        qu.question.time), int(qu.question.id)])
                ...
                messages.error(
                    request, 'Congratulations, You have completed the video interview. We will be in touch shortly. Thank you.')
        ...
        if isinstance(interview_instance, TakenInterview):
            if 'candidate_ID' in request.GET and 'interview_ID' in request.GET and 'question_ID' in request.GET \
                ...
                if int(request.GET['interview_ID']) != kwrgs['pk'] or int(
                    ...
                    interview_instance.save()
                    answer_instance = interview_instance.answers.filter(
                        question__id=int(request.GET['question_ID']))[0]
                    ...
                    response = HttpResponse(
                        'Details saved Successfully!', status=200)
                    response['Content-Length'] = len(response.content)
                    return response
                ...
                if int(request.GET['interviewid']) != kwrgs['pk'] or int(
                    ...
                    interview_instance.save()
                    messages.success(
                        request, "Interview completed successfully!!")
                    return redirect('interviewee:dashboard')
        else:
            response = HttpResponse('User rights require', status=404)
            response['Content-Length'] = len(response.content)
    ...
    if request.user.user_type == 10:
        ...
        if pending_practice == 0 and pending_interview > 0:
            ...
            if len(interview_instance) > 0:
                ...
                interviewee = Interviewee.objects.filter(
                    user_for_interview__email=request.user.email)[0]
            ...
            if totalquestion > 0:
                ...
                interview_instance.save()
                messages.error(
                    request, 'Congratulations, You have attempted all interviews!')
                return redirect('interviewee:dashboard')

        else:
            messages.info(
                request, 'You have already attempted practice questions!')
            return redirect('interviewee:dashboard')
    else:
        response = HttpResponse('Page not found', status=404)
        response['Content-Length'] = len(response.content)
